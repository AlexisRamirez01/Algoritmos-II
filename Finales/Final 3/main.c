...
{
    line_iterator it = line_iterator_begin(line);
    unsigned int res = 0u;
    while(!line_iterator_end(it)){
        if (domino_is_double(line_iterator_get(it)))
        {
            res += 1;
        }
...
bool is_in_line(domino_line line, domino tile)
{
    ...
    line_iterator it = line_iterator_begin(line);
    while(!line_iterator_end(it)){
        found = domino_up(line_iterator_get(it)) == domino_up(tile) && 
                domino_down(line_iterator_get(it)) == domino_down(tile);
        it = line_iterator_next(it);
    }
